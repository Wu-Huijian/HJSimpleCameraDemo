...
@interface HJCaptureSessionManager ()
...
@implementation HJCaptureSessionManager
...
- (void)addVideoInputFrontCamera:(BOOL)front {
    ...
    for (AVCaptureDevice *device in devices) {
        ...
        WLog(@"Device name: %@", [device localizedName]);
        
        if ([device hasMediaType:AVMediaTypeVideo]) {
            
            if ([device position] == AVCaptureDevicePositionBack) {
                WLog(@"Device position : back");
                backCamera = device;
                
            }  else {
                WLog(@"Device position : front");
                frontCamera = device;
            }
        }
    }
    ...
    if (front) {
        ...
        if (!error) {
            if ([_session canAddInput:frontFacingCameraDeviceInput]) {
                ...
                WLog(@"Couldn't add front facing video input");
            }
        }
    } else {
        ...
        if (!error) {
            if ([_session canAddInput:backFacingCameraDeviceInput]) {
                ...
                WLog(@"Couldn't add back facing video input");
            }
        }
    }
}
...
//    [videoConnection setVideoOrientation:avcaptureOrientation];
	...
	WLog(@"about to request a capture from: %@", _stillImageOutput);
    
    [_stillImageOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
        
        CFDictionaryRef exifAttachments = CMGetAttachment(imageDataSampleBuffer, kCGImagePropertyExifDictionary, NULL);
        if (exifAttachments) {
            WLog(@"attachements: %@", exifAttachments);
        } else {
            WLog(@"no attachments");
        }
        NSData *imageData = [AVCaptureStillImageOutput jpegStillImageNSDataRepresentation:imageDataSampleBuffer];
        UIImage *image = [[UIImage alloc] initWithData:imageData];
        WLog(@"originImage:%@", [NSValue valueWithCGSize:image.size]);
...
//        } else {
            if (device.flashMode == AVCaptureFlashModeOff) {
                ...
                imgStr = @"netScan_light_on";
                
            } else if (device.flashMode == AVCaptureFlashModeOn) {
                device.flashMode = AVCaptureFlashModeOff;
                imgStr = @"netScan_light_off";
            }
...
//        }
        ...
        if (sender) {
            [sender setImage:[UIImage imagesNamedFromCustomBundle:imgStr] forState:UIControlStateNormal];
        }
...
- (void)switchGrid:(BOOL)toShow {
    ...
    CGFloat headHeight = _previewLayer.bounds.size.height;
    CGFloat squareLength = ScreenWidth;
    CGFloat eachAreaLength = squareLength / 3.f;
    CGFloat verticalHeight = ScreenHeight/3.f;
    
    for (int i = 0; i < 4; i++) {
        CGRect frame = CGRectZero;
        if (i == 0 || i == 1) {//画横线
            frame = CGRectMake(0, verticalHeight*(i+1), squareLength, 0.5);
        } else {
            frame = CGRectMake((i + 1 - 2) * eachAreaLength, 0, 0.5, headHeight);
        }
        [HJCommon drawALineWithFrame:frame andColor:[UIColor whiteColor] inLayer:_preview.layer];
    }
}
